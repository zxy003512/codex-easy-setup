...
on_error() {
  local exit_code=$?
  printf '%s\n' "[错误] 脚本在第 $1 行失败（退出码 $exit_code）。没有显示或记录 API Key。" >&2
  exit "$exit_code"
}
...
strip_managed_block() {
  ...
  printf '%s\n' 'case ":$PATH:" in' >> "$temp"
  printf '%s\n' '  *":$HOME/.local/bin:"*) ;;' >> "$temp"
  printf '%s\n' '  *) export PATH="$HOME/.local/bin:$PATH" ;;' >> "$temp"
  printf '%s\n' 'esac' >> "$temp"
...
ask_secret() {
  ...
  IFS= read -r -s answer
  printf '\n' >&2
  is_single_line "$answer" || die "API Key 不能包含换行符。"
  printf -v "$variable" '%s' "$answer"
}
...
ask_reasoning() {
  ...
  local answer
  while true; do
    printf '模型思考强度 minimal / low / medium / high / xhigh [%s]: ' "$default_value" >&2
    IFS= read -r answer
    answer=${answer:-$default_value}
    case $answer in
      minimal|low|medium|high|xhigh)
        ...
        return 0
        ;;
      *) warn "请输入 minimal、low、medium、high 或 xhigh。" ;;
    esac
  done
}
...
configure_noninteractive() {
  ...
  case $CODEX_EASY_REASONING_EFFORT in minimal|low|medium|high|xhigh) ;; *) die "无效的 CODEX_EASY_REASONING_EFFORT。" ;; esac
